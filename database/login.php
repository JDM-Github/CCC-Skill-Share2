...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT id, verified, fullName, email, password, program, yearLevel, skills, photoUrl, resumePdf, isCompany FROM users WHERE email = ?");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $stmt->bind_result($userId, $verified, $fullName, $userEmail, $hashedPassword, $program, $yearLevel, $skillsJson, $photoUrl, $resumePdf, $aCompany);
    $stmt->fetch();
    $stmt->close();
